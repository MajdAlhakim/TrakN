...
//
// Hardware: Beetle ESP32-C6 + MPU6050 IMU + BMP180 barometer + Beetle ESP32-C6
// scanner (via UART) Board package: esp32 by Espressif Systems v3.3.5+ Board
// selection: Tools → Board → esp32 → ESP32C6 Dev Module USB CDC On Boot:
// Enabled
//
  ...
  while (1) {
    ...
    if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(5)) == pdTRUE) {
      ...
      if (n == 14) {
        ...
        Wire.read();
        Wire.read(); // skip temperature registers
    ...
    if ((ok_count + err_count) % 200 == 0)
      Serial.printf("[IMU] ok=%lu err=%lu batch=%d\n", (unsigned long)ok_count,
                    (unsigned long)err_count, imu_batch_idx);
...
static void baro_init() {
  ...
  if (count > 0) {
    ...
    baro_ok = false;
    Serial.println(
        "[BARO] baseline calibration failed — floor detection disabled");
  }
}
  ...
  while (1) {
    ...
    if (xSemaphoreTake(i2c_mutex, pdMS_TO_TICKS(30)) == pdTRUE) {
      ...
      if (P > 0 && baselinePressure > 0.0f) {
        altDelta =
            44330.0f * (1.0f - powf((float)P / baselinePressure, 0.1903f));
        ok = true;
      }
    }
    ...
    if (ok) {
      ...
      if (altDelta < -2.0f)
        newFloor = -1; // Basement
      else if (altDelta < 2.5f)
        newFloor = 0; // Ground
      else if (altDelta < FLOOR_HEIGHT_M + 3.0f && altDelta > 2.5f)
        newFloor = 1; // Floor 1
      else
        newFloor = 2; // Floor 2
      ...
      Serial.printf("[BARO] dAlt=%.2fm floor=%d (pending=%d votes=%d/%d)\n",
                    altDelta, estimatedFloor, pendingFloor, floorVoteCount,
                    BARO_CONFIRM_THRESHOLD);
    }
...
static void connect_to_best_ap() {
  ...
  if (xSemaphoreTake(ap_mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    for (int i = 0; i < ap_count; i++) {
      if (strcmp(ap_list[i].ssid, WIFI_SSID) != 0)
        continue;
  ...
  uint8_t bssid[6];
  sscanf(best.bssid, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &bssid[0], &bssid[1],
         &bssid[2], &bssid[3], &bssid[4], &bssid[5]);

  Serial.printf("[WIFI] targeting bssid=%s rssi=%d ch=%d\n", best.bssid,
                best.rssi, best.channel);
...
static bool current_bssid_in_scan() {
  ...
  char cur_str[18];
  snprintf(cur_str, sizeof(cur_str), "%02X:%02X:%02X:%02X:%02X:%02X", cur[0],
    ...
    if (!wifi_connected) {
      ...
      if (seq != last_processed_scan_seq) {
        ...
        if (!current_bssid_in_scan()) {
          Serial.println("[WIFI] current BSSID missing from new scan — jumping "
                         "to strongest");
          connect_to_best_ap();
        }
      }
...
static void post_task(void *arg) {
  ...
  while (1) {
    ...
    Serial.printf("[POST] RSSI=%d IP=%s floor=%d\n", WiFi.RSSI(),
                  WiFi.localIP().toString().c_str(), current_floor);
...
void setup() {
  ...
  imu_init();  // Wire.begin(19,20), MPU6050 config — no tasks yet, no mutex
               // needed
  baro_init(); // BMP180 init + Ground Floor baseline — no tasks yet, no mutex
               // needed
